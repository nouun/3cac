...
import { fetchAltRank, fetchBubbles, fetchGalaxyScore, fetchVolatility } from "./lib.js";
...
if (validateConfig(config)) {
    ...
    const promises = bots
      .map(async (bot) => {
        const botConfig = config.bots
          ?.filter((botConfig) =>
            new RegExp(botConfig.name).test(bot.name))[0] ?? null;
  ...
  const checkMaxMin = (value: number, maxMin?: MaxMin): boolean =>
  ...
  const run = async () => {
    ...
    for (const { bot, config, marketCode } of bots) {
      const botConfig = <BotOptions>config;
      if (progConfig.readOnly) console.log(`++ Getting pairs for ${bot.name}`);
      else console.log(`++ Updating ${bot.name}`);

      if (!validPairs[marketCode]) {
        console.log(`-- Fetching valid pairs for ${marketCode}`);
        validPairs[marketCode] = <string[]>await api.getMarketPairs({ market_code: marketCode });
      }

      let bubblePairs: string[] = bubbles
        .filter((bubble) => {
        ...
        .sort((bubble1, bubble2) =>
          bubble2.data.usd.performance[botConfig.filter?.performance?.sort ?? "hour"] -
      ...
      const baseToPair = (base: string): string => `${botConfig.pairs.quote}_${base}`;
  ...
  const loopWithDelay = async (delay: number) => {
    while (true) {
      await run();
